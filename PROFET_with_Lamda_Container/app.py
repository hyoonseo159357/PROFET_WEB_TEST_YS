...
for anchor_instance in INSTANCE_LIST:
    ...
    for pred_instance in pred_instance_list:
        ...
        model_linear = pickle.load(open(f'/var/task/PROFET_WEB_TEST_YS/Model/anchor_{anchor_name}_{pred_name}_linear.model', 'rb'))
        model_rfr = pickle.load(open(f'/var/task/PROFET_WEB_TEST_YS/Model/anchor_{anchor_name}_{pred_name}_rfr.model', 'rb'))
        model_dnn = tf.keras.models.load_model(f'/var/task/PROFET_WEB_TEST_YS/Model/anchor_{anchor_name}_{pred_name}_dnn.model')
...
def feature_clustering(json_feature):
...
def lambda_handler(event, context):
    ...
    body = body.replace('\r',"")
    body = body.replace('-',"")
    test_x = feature_clustering(json_feature)
